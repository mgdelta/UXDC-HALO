...
Adafruit_DotStar::Adafruit_DotStar(uint16_t n, uint8_t o) :
 ...
 rOffset(o & 3), gOffset((o >> 2) & 3), bOffset((o >> 4) & 3), spi_fdescriptor(-1)
{
  updateLength(n);
  std::cout << "Konstruktor HW SPI" << std::endl;
}
...
/*!
  @brief   Switch over to 'soft' (bit#include <fcntl.h>bang) SPI. DotStars can be connected
...
void Adafruit_DotStar::hw_spi_init(void) { // Initialize hardware SPI
	...
	int spi_fd = -1;
	spi_fd = wiringPiSPISetup(0, 1000000);
	if(spi_fd < 0) {
		std::cout << "WiringPiSPISetup failed!" << std::endl;
	}
	spi_fdescriptor = spi_fd;
...
#endif
#define spi_out(n) hw_spi_out(n)
//#define spi_out(n) std::cout << "Hallo" //wiringPiSPIDataRW(0, (unsigned char) n, 1)
...
void Adafruit_DotStar::hw_spi_out(uint8_t n) {
	
	if (spi_fdescriptor > 0)
	write(spi_fdescriptor, &n, 1);
	
	//wiringPiSPIDataRW(0, &n, 1);
	//std::cout << "SPI";
}
...
void Adafruit_DotStar::Halo_ClearAll()
{
	clear();
	show();
}

void Adafruit_DotStar::Halo_Strip_FadeIn(const uint32_t color, const uint8_t target_brightness, const unsigned int fadetime_ms)
{
	fill(color,0, numLEDs);
	for (int i = 0; i < 128; i+=1)
	{
		uint8_t bright_value = _MyDotStarSineTable[i];
		if (bright_value < target_brightness)
		{
...
void Adafruit_DotStar::Halo_Strip_FadeOut(const uint32_t color, const unsigned int fadetime_ms)
{
	uint8_t starting_brightness = getBrightness();
	int sine_start_value = 0;
	for (int i = 0; i < 128; i++)
	{
		uint8_t bright_value = _MyDotStarSineTable[i];
		if (bright_value <= starting_brightness)
			sine_start_value = i;
	}
	
	for (int i = sine_start_value; i > 0; i--)
	{
		uint8_t bright_value = _MyDotStarSineTable[i];
		setBrightness(bright_value);
		show();
